...
#include "HMT_SpinQuant_Prefilling_mem_opt.h"
...
template <int hmt_t_block_parallel, int weight_parallel, int input_dim = HIDDEN_DIM, int output_dim = HIDDEN_DIM>
void hmt_read_fp32_bin_as_fp32_weight_mmap(
    const std::string& layer_name,
    std::vector<hls::vector<float, weight_parallel>, tapa::aligned_allocator<hls::vector<float, weight_parallel>>> weight_mmaps[hmt_t_block_parallel],
    int mmap_offset = 0,
    ...
    // Each iteration packs two output channels: (2*n, 2*n+1)
    for (int block_id = 0; block_id < hmt_t_block_parallel; ++block_id) {
        for (int n = 0; n < output_dim/hmt_t_block_parallel; ++n) {
            // read one whole row (input_dim bytes) for each of the two output channels
            std::vector<float> data(input_dim);
            const std::streamoff off = static_cast<std::streamoff>(block_id * (output_dim/hmt_t_block_parallel) + n) * input_dim * sizeof(float);

            fin.seekg(off, std::ios::beg);
            fin.read(reinterpret_cast<char*>(data.data()), data.size() * sizeof(float));
            if (!fin) { std::cerr << "Read error @ row " << block_id * (output_dim/hmt_t_block_parallel) + n << " in " << name << "\n"; return; }

            // Pack and write to mmap
            const int tile  = (n + out_dim_offset)  / (weight_parallel);
            const int lane  = (n + out_dim_offset)  % (weight_parallel);
            for (int m = 0; m < input_dim; ++m) {
                weight_mmaps[block_id][mmap_offset + tile * input_dim + m][lane] = data[m];
            }
        }
    }
}

template <int hmt_t_block_parallel, int hidden_dim=HIDDEN_DIM, int max_seq_len = HMT_MAX_SEG_NUM * HMT_SEG_LEN>
void hmt_prefilling_read_embedding_from_bin(
    const std::string& token_embed_binfile_name,
    const std::string& Tn_embed_binfile_name,
    std::vector<int> token_idx,
    std::vector<hls::vector<float, hmt_t_block_parallel>, tapa::aligned_allocator<hls::vector<float, hmt_t_block_parallel>>>& hmt_io_mmap,
    int mmap_offset,
    ...
    for (size_t idx = 0; idx < hmt_io_mmap.size(); ++idx) {
        hmt_io_mmap[idx] = 0.0f;
    }

    // Read Tn embedding
    char Tn_name[256];
    std::snprintf(Tn_name, sizeof(Tn_name), "parameters/%s.bin", Tn_embed_binfile_name.c_str());
    std::ifstream Tn_fin(Tn_name, std::ios::binary);
    if (!Tn_fin) {
        std::cerr << "Failed to open file: " << Tn_name << "\n";
        return;
    }
    std::vector<float> Tn_data(hidden_dim);
    Tn_fin.seekg(0, std::ios::beg);
    Tn_fin.read(reinterpret_cast<char*>(Tn_data.data()), Tn_data.size() * sizeof(float));
    if (!Tn_fin) { std::cerr << "Read error @ Tn in " << Tn_name << "\n"; return; } 
    // Pack and write to mmap
    for (int m = 0; m < hidden_dim; ++m) {
        hmt_io_mmap[mmap_offset + m % (hidden_dim/hmt_t_block_parallel)][m / (hidden_dim/hmt_t_block_parallel)] = Tn_data[m];
    }
    Tn_fin.close();

    // Read token embeddings
    char name[256];
    std::snprintf(name, sizeof(name), "parameters/%s.bin", token_embed_binfile_name.c_str());
    std::ifstream fin(name, std::ios::binary);
    if (!fin) {
        std::cerr << "Failed to open file: " << name << "\n";
        return;
    ...
    for (int n = 0; n < token_num; ++n) {
        ...
        // Pack and write to mmap
        for (int m = 0; m < hidden_dim; ++m) {
            hmt_io_mmap[mmap_offset + (1 + n) * (hidden_dim/hmt_t_block_parallel) + m % (hidden_dim/hmt_t_block_parallel)][m / (hidden_dim/hmt_t_block_parallel)] = data[m];
        }
    }
    fin.close();
}

void HMT_SpinQuant_Prefilling_test(int argc, char* argv[]) {
    ...
    cout << "Pre FFN Weight Block Num: " << PRE_FFN_W_BLOCK_NUM << endl;

    // Input/Output mmap
    vector<hls::vector<float, HMT_T_BLOCK_PARALLEL>, tapa::aligned_allocator<hls::vector<float, HMT_T_BLOCK_PARALLEL>>> hmt_io_mmap((1 + HMT_MAX_SEG_NUM * HMT_SEG_LEN) * HIDDEN_DIM / HMT_T_BLOCK_PARALLEL);
    vector<hls::vector<float, TOKEN_PARALLEL>, tapa::aligned_allocator<hls::vector<float, TOKEN_PARALLEL>>> pref_io_mmap((DECODER_LAYER_NUM + 1) * MAX_PRE_SEQ_LEN/TOKEN_PARALLEL * HIDDEN_DIM);


    //HMT weight mmap
    vector<hls::vector<float, HMT_W_QK_ATTN_PARALLEL>, tapa::aligned_allocator<hls::vector<float, HMT_W_QK_ATTN_PARALLEL>>> hmt_wq_wk_mmaps[HMT_T_BLOCK_PARALLEL];
    for(int i = 0; i < HMT_T_BLOCK_PARALLEL; i++){
        hmt_wq_wk_mmaps[i].resize(HIDDEN_DIM/(HMT_T_BLOCK_PARALLEL * HMT_W_QK_ATTN_PARALLEL) * HIDDEN_DIM);
    }
    ...
    // MHA
    vector<hls::vector<ap_int<8>, PRE_K_PARALLEL>, tapa::aligned_allocator<hls::vector<ap_int<8>, PRE_K_PARALLEL>>> k_cache(
        DECODER_LAYER_NUM * KV_HEAD_NUM * MAX_PRE_SEQ_LEN/PRE_K_PARALLEL * HEAD_DIM
    );
    cout << "k_cache size: " << k_cache.size() << endl;

    vector<hls::vector<ap_int<8>, PRE_V_PARALLEL>, tapa::aligned_allocator<hls::vector<ap_int<8>, PRE_V_PARALLEL>>> v_cache(
        DECODER_LAYER_NUM * KV_HIDDEN_DIM/PRE_V_PARALLEL * MAX_PRE_SEQ_LEN
    );
    cout << "v_cache size: " << v_cache.size() << endl;
    ...
    for(int i = 0; i < HMT_T_BLOCK_PARALLEL; i++){
        initFloatVec(hmt_wq_wk_mmaps[i], HMT_W_QK_ATTN_PARALLEL);
    }
    ...
    // for(int i = 0; i < PRE_FFN_W_BLOCK_NUM; ++i) initInt8Vec(w_ffn_gate_mmaps[i], PRE_FFN_W_PARALLEL_READ/PRE_FFN_W_BLOCK_NUM/2);
    // initFloatVec(w_ffn_gate_s_sum_mmap, 2);
    // for(int i = 0; i < PRE_FFN_W_BLOCK_NUM; ++i) initInt8Vec(w_ffn_up_mmaps[i], PRE_FFN_W_PARALLEL_READ/PRE_FFN_W_BLOCK_NUM/2);
    // initFloatVec(w_ffn_up_s_sum_mmap, 2);
    // for(int i = 0; i < PRE_FFN_W_BLOCK_NUM; ++i) initInt8Vec(w_ffn_down_mmaps[i], PRE_FFN_W_PARALLEL_READ/PRE_FFN_W_BLOCK_NUM/2);
    ...
    // hmt_read_fp32_bin_as_fp32_weight_mmap<HMT_T_BLOCK_PARALLEL, HMT_W_QK_ATTN_PARALLEL, HIDDEN_DIM, HIDDEN_DIM>(
    //     "hmt_q_proj", hmt_wq_wk_mmaps, 0
    // );
    // hmt_read_fp32_bin_as_fp32_weight_mmap<HMT_T_BLOCK_PARALLEL, HMT_W_QK_ATTN_PARALLEL, HIDDEN_DIM, HIDDEN_DIM>(
    //     "hmt_k_proj", hmt_wq_wk_mmaps, HIDDEN_DIM/(HMT_T_BLOCK_PARALLEL * HMT_W_QK_ATTN_PARALLEL) * HIDDEN_DIM
    // );
    ...
    // Apply initialization
        ...
        for (size_t idx = 0; idx < pref_io_mmap.size(); ++idx) {
        ...
        // hmt_prefilling_read_embedding_from_bin<HMT_T_BLOCK_PARALLEL, HIDDEN_DIM>(
        //     "model_embed_tokens_fp32", "tn_embed_fp32", token_idx, hmt_io_mmap, 0
        // );
        hmt_prefilling_read_embedding_from_bin<HMT_T_BLOCK_PARALLEL, HIDDEN_DIM>(
            "model_embed_tokens_fp32", "model_embed_tokens_fp32", token_idx, hmt_io_mmap, 0
        );
        ...
        int64_t kernel_time_ns = tapa::invoke(
            ...
            FLAGS_bitstream,
            tapa::read_write_mmap<hls::vector<float, HMT_T_BLOCK_PARALLEL>>(hmt_io_mmap),
            tapa::read_only_mmaps<hls::vector<float, HMT_W_QK_ATTN_PARALLEL>, HMT_T_BLOCK_PARALLEL>(hmt_wq_wk_mmaps),
...
int main(int argc, char* argv[]) {
    HMT_SpinQuant_Prefilling_test(argc, argv);
    return 0;
}