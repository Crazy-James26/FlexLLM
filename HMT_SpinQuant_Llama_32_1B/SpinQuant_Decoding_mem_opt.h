...
void dec_weight_loader_qkvo_FFN(
    ...
    decoder_seq_loop: for (int dec_seq_id = 0; dec_seq_id < dec_seq_len; dec_seq_id++){
        decoder_block_loop: for (int block_id = 0; block_id < DECODER_LAYER_NUM; block_id++){
            ...
            //     weight_mmap, weight_stream, block_id, HIDDEN_DIM, KV_HIDDEN_DIM, addr_bias + w_k_addr_bias
            // );//for k

            // dec_weight_loader_int4_pack_2<T_QKVO_FFN_BLOCK_PARALLEL, DEC_QKVO_FFN_W_PARALLEL, INTER_DIM, INTER_DIM>(
            //     weight_mmap, weight_stream, block_id, HIDDEN_DIM, KV_HIDDEN_DIM, addr_bias + w_v_addr_bias
            // );// for v

            dec_weight_loader_int4_pack_2<T_QKVO_FFN_BLOCK_PARALLEL, DEC_QKVO_FFN_W_PARALLEL, INTER_DIM, INTER_DIM>(
                weight_mmap, weight_stream, block_id, HIDDEN_DIM, KV_HIDDEN_DIM_PAD, addr_bias + w_kv_addr_bias
            ); // for k and v, block 0~3: K, block 4~7: V

            dec_weight_loader_int4_pack_2<T_QKVO_FFN_BLOCK_PARALLEL, DEC_QKVO_FFN_W_PARALLEL, INTER_DIM, INTER_DIM>(
                weight_mmap, weight_stream, block_id, HIDDEN_DIM, HIDDEN_DIM, addr_bias + w_q_addr_bias
            );// for q

            dec_weight_loader_int4_pack_2<T_QKVO_FFN_BLOCK_PARALLEL, DEC_QKVO_FFN_W_PARALLEL, INTER_DIM, INTER_DIM>(
                weight_mmap, weight_stream, block_id, HIDDEN_DIM, HIDDEN_DIM, addr_bias + w_o_addr_bias
            ); //for o
        ...
        dec_weight_loader_int4_pack_2<T_QKVO_FFN_BLOCK_PARALLEL, DEC_QKVO_FFN_W_PARALLEL, INTER_DIM, INTER_DIM>(
            weight_mmap, weight_stream, 0, HIDDEN_DIM, VOCAB_SIZE_PAD, addr_bias + w_vocab_addr_bias
        ); // for vocab
    }
}
...
void dec_Linear_Layer_i4xi4_qkvo_FFN_input_broadcastor(
    tapa::istream<hls::vector<ap_int<4>, T_QKVO_FFN_BLOCK_PARALLEL>>& input_stream,
    tapa::ostreams<hls::vector<ap_int<4>, T_QKVO_FFN_BLOCK_PARALLEL>, 2>& input_streams_half,
    int dec_seq_len
){
    decoder_seq_loop: for (int dec_seq_id = 0; dec_seq_id < dec_seq_len; dec_seq_id++){
        decoder_block_loop: for (int block_id = 0; block_id < DECODER_LAYER_NUM; block_id++){
...
void dec_Linear_Layer_i4xi4_qkvo_FFN_half(
    tapa::istream<hls::vector<ap_int<4>, T_QKVO_FFN_BLOCK_PARALLEL>>& input_stream,
    tapa::istreams<hls::vector<ap_int<4>, DEC_QKVO_FFN_W_PARALLEL>, T_QKVO_FFN_BLOCK_PARALLEL/2>& weight_streams,
    tapa::ostream<hls::vector<ap_int<log2_INTER_DIM + 8>, T_QKVO_FFN_BLOCK_PARALLEL/2>>& output_stream,
    int dec_seq_len
){
    decoder_seq_loop: for (int dec_seq_id = 0; dec_seq_id < dec_seq_len; dec_seq_id++){
        decoder_block_loop: for (int block_id = 0; block_id < DECODER_LAYER_NUM; block_id++){
...
void dec_Linear_Layer_i4xi4_qkvo_FFN_output_merger(
    tapa::istreams<hls::vector<ap_int<log2_INTER_DIM + 8>, T_QKVO_FFN_BLOCK_PARALLEL/2>, 2>& output_streams_half,
    tapa::ostream<hls::vector<ap_int<log2_INTER_DIM + 8>, T_QKVO_FFN_BLOCK_PARALLEL>>& output_stream,
    int dec_seq_len
){
    decoder_seq_loop: for (int dec_seq_id = 0; dec_seq_id < dec_seq_len; dec_seq_id++){
        decoder_block_loop: for (int block_id = 0; block_id < DECODER_LAYER_NUM; block_id++){
            dec_stream_block_parallel_merger<ap_int<log2_INTER_DIM + 8>, T_QKVO_FFN_BLOCK_PARALLEL, 2, INTER_DIM>(
                output_streams_half, output_stream, KV_HIDDEN_DIM_PAD
            ); // for k and v
            dec_stream_block_parallel_merger<ap_int<log2_INTER_DIM + 8>, T_QKVO_FFN_BLOCK_PARALLEL, 2, INTER_DIM>(
        ...
        dec_stream_block_parallel_merger<ap_int<log2_INTER_DIM + 8>, T_QKVO_FFN_BLOCK_PARALLEL, 2, INTER_DIM>(
            output_streams_half, output_stream, VOCAB_SIZE_PAD
        ); // for vocab
    }
}
...
void SpinQuant_Decoding(
    tapa::mmap<hls::vector<float, T_BLOCK_PARALLEL>> vocab_lib,
    tapa::mmap<hls::vector<float, T_BLOCK_PARALLEL>> io_mmap,
    // tapa::mmaps<hls::vector<ap_int<4>, DEC_QKVO_FFN_W_PARALLEL>, T_QKVO_FFN_BLOCK_PARALLEL> w_qkvo_FFN_mmaps,
    // tapa::mmaps<hls::vector<ap_int<4>, DEC_QKVO_FFN_W_PARALLEL>, T_QKVO_FFN_BLOCK_PARALLEL/2> w_qkvo_FFN_mmaps_half_0,
    // tapa::mmaps<hls::vector<ap_int<4>, DEC_QKVO_FFN_W_PARALLEL>, T_QKVO_FFN_BLOCK_PARALLEL/2> w_qkvo_FFN_mmaps_half_1,
    tapa::mmaps<hls::vector<ap_int<8>, DEC_QKVO_FFN_W_PARALLEL/2>, T_QKVO_FFN_BLOCK_PARALLEL/2> w_qkvo_FFN_mmaps_half_0_k_caches,
    tapa::mmaps<hls::vector<ap_int<8>, DEC_QKVO_FFN_W_PARALLEL/2>, T_QKVO_FFN_BLOCK_PARALLEL/2> w_qkvo_FFN_mmaps_half_1_v_caches,
    ...
    tapa::streams<hls::vector<ap_int<4>, T_QKVO_FFN_BLOCK_PARALLEL>, 2> quant_input_qkvo_ffn_streams_half("quant_input_qkvo_ffn_streams_half");

    // tapa::streams<hls::vector<ap_int<4>, DEC_QKVO_FFN_W_PARALLEL>, T_QKVO_FFN_BLOCK_PARALLEL> w_qkvo_ffn_streams("w_qkvo_ffn_streams");
    tapa::streams<hls::vector<ap_int<4>, DEC_QKVO_FFN_W_PARALLEL>, T_QKVO_FFN_BLOCK_PARALLEL/2> w_qkvo_ffn_streams_half_0("w_qkvo_ffn_streams_half_0");
    tapa::streams<hls::vector<ap_int<4>, DEC_QKVO_FFN_W_PARALLEL>, T_QKVO_FFN_BLOCK_PARALLEL/2> w_qkvo_ffn_streams_half_1("w_qkvo_ffn_streams_half_1");
    tapa::stream<hls::vector<float, 2>> w_s_sum_qkvo_ffn_stream("w_s_sum_qkvo_ffn_stream");

    tapa::streams<hls::vector<ap_int<log2_INTER_DIM + 8>, T_QKVO_FFN_BLOCK_PARALLEL/2>, 2> quant_output_qkvo_ffn_streams_half("quant_output_qkvo_ffn_streams_half");
    ...
    .invoke(dec_Linear_Layer_i4xi4_qkvo_FFN_input_broadcastor, quant_input_qkvo_ffn_stream, quant_input_qkvo_ffn_streams_half, dec_seq_len)
    .invoke(dec_Linear_Layer_i4xi4_qkvo_FFN_half, quant_input_qkvo_ffn_streams_half[0], w_qkvo_ffn_streams_half_0, quant_output_qkvo_ffn_streams_half[0], dec_seq_len)
    .invoke(dec_Linear_Layer_i4xi4_qkvo_FFN_half, quant_input_qkvo_ffn_streams_half[1], w_qkvo_ffn_streams_half_1, quant_output_qkvo_ffn_streams_half[1], dec_seq_len)
    .invoke(dec_Linear_Layer_i4xi4_qkvo_FFN_output_merger, quant_output_qkvo_ffn_streams_half, quant_output_qkvo_ffn_stream, dec_seq_len)
