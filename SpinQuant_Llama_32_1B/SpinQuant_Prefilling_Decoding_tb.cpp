...
void SpinQuant_Prefilling_Decoding_test(int argc, char* argv[]) {
    ...
    cout << "k_caches size: " <<  dec_k_caches[0].size() << endl;
    ...
    cout << "v_caches size: " <<  dec_v_caches[0].size() << endl;
    ...
    // Apply initialization
        ...
        int run_pre_len[num_runs] = {512, 512, 512, 1024, 1024, 1024};
        int run_dec_len[num_runs] = {256, 512, 1024, 256, 512, 1024};
                ...
                for (int h = 0; h < KV_HEAD_NUM; h++) {
                    for (int j = 0; j < HEAD_DIM; j++) {
                        ...
                        if(i < MAX_PRE_SEQ_LEN){
                            dec_k_caches[h/(KV_HEAD_NUM/DEC_HEAD_PARALLEL)][write_idx][write_sub_idx] = pref_k_cache[read_idx][read_sub_idx];
                        }
                        else {
                            dec_k_caches[h/(KV_HEAD_NUM/DEC_HEAD_PARALLEL)][write_idx][write_sub_idx] = ap_int<8>(0);
                        }
                        ...
                        if(i < MAX_PRE_SEQ_LEN){
                            dec_v_caches[h/(KV_HEAD_NUM/DEC_HEAD_PARALLEL)][write_idx][write_sub_idx] = pref_v_cache[read_idx][read_sub_idx];
                        }
                        else {
                            dec_v_caches[h/(KV_HEAD_NUM/DEC_HEAD_PARALLEL)][write_idx][write_sub_idx] = ap_int<8>(0);
                        }
                    }
                }
            }
...
int main(int argc, char* argv[]) {
    SpinQuant_Prefilling_Decoding_test(argc, argv);
    return 0;
}